...
get_papers <- function(query, params, limit=100) {
  ...
  query = 'SELECT ?item ?itemLabel ?_PubMed_ID ?PMCID ?publication_date ?published_in ?published_inLabel
(group_concat(distinct ?author_name_string;separator="; ") as ?authors) 
  WHERE {
  {
  SELECT * WHERE {
  ?item wdt:P31 wd:Q13442814 ; #scholarly 
  ...
  OPTIONAL { ?item wdt:P1433 ?published_in. }
  } GROUP BY ?item ?itemLabel ?_PubMed_ID ?PMCID ?publication_date ?published_in ?published_inTitle
  LIMIT 100'
  ...
  names(metadata)[names(metadata)=="title"] <- "itemLabel"
